...
export function buildPipelineStatusReport(db: Database.Database): string {
  ...
  const lines = [
    ...
    '',
    `Counts: ${counts.source_items} sources, ${counts.insights} insights, ${counts.opportunities} opportunities, ${activeOpportunities} active, ${archivedOpportunities} archived, ${counts.runs_pending} open research runs, ${counts.artifacts} artifacts`,
    '',
    'Stages:',
  ];
...
export function buildOpportunityExplanation(
  ...
  const lines = [
    ...
    `Lifecycle: ${formatLifecycleStage(opportunity.lifecycle_stage)} | Status: ${opportunity.status}`,
    ...
    `- Learned adjustment: ${taste.learnedAdjustment}`,
    `- Founder preference adjustment: ${taste.preferenceAdjustment}`,
    `- Founder preference signals: ${taste.preferenceSignals.map((signal) => `${signal.signal}=${signal.score}`).join(', ') || 'none'}`,
